...
int PWM_speed_control(h_shell_t *h_shell, char **argv, int argc)
{
	...
	if (argc != 2)
	{
		...
		HAL_UART_Transmit(&huart4, error_message, sizeof(error_message),
				HAL_MAX_DELAY);
	...
	int uartTxStringLength = snprintf((char*) h_shell->print_buffer,
			BUFFER_SIZE, "speed set to %lu of max value \r\n",
			(unsigned long) speed);
	HAL_UART_Transmit(&huart4, h_shell->print_buffer, uartTxStringLength,
			HAL_MAX_DELAY);
	return 0;
