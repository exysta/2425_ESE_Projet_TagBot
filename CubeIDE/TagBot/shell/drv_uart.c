...
/**
 * @brief UART receive complete callback, triggered by HAL on RX interrupt.
 *
 * This function is called automatically when a receive interrupt occurs on the
 * UART. It notifies the task waiting for RX completion and optionally performs
 * a context switch if a higher priority task was woken.
 *
...
/**
 * @brief UART transmit complete callback, triggered by HAL on TX interrupt.
 *
 * This function is called automatically when a transmit interrupt occurs on the
 * UART. It notifies the task waiting for TX completion and optionally performs
 * a context switch if a higher priority task was woken.
 *
 * @param huart Pointer to the UART handle structure.
 */
